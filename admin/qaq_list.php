...
<script>
    ...
    layui.use('table', function(){
        ...
        table.render({
            elem: '#LAY_table_user'
            ,url: 'php/qaq/qaq_query.php'
            ,method: 'get'
            ,where: {xx:5 }
            ,cols: [[
                {field:'id', title: 'qaqID', sort: true, fixed: false,width:100}
                ,{field:'title', title: 'qaq标题', sort: false, fixed: false,width:300}
                ,{field:'summary', title: 'qaq摘要文字',  sort: false, fixed: false}  
                ,{field:'click', title: '点击数量', sort: false, fixed: false,width:100}
...
//js实现条件搜索传值的地方
        ...
        table.on('tool(useruv)', function(obj){
            ...
            if(obj.event === 'edit'){
                ...
                var c='php/qaq/qaq_edit.php?qaq_id='+data.id;
                x_admin_show('qaq编辑',c,1200,810);
             	 
             	               
            } else if(obj.event === 'del'){
                layer.confirm('确定删除这条qaq?', function(index){
                    console.log(data);
                    obj.del();
                    layer.close(index);
                    $.ajax({
                        url: "php/qaq/qaq_delete.php",
                        type: "post",
                        data:{"id":data.id},
                        dataType: "text",
