...
if (isset($file_pc_url)) {
    $sql_pc = "UPDATE cptp_product SET product_img_url='{$img_pc_url}' WHERE product_id='{$product_id}'";
    $sqlfinish = mysqli_query($link, $sql_pc);
}

if (isset($file_wap_url)) {
    $sql_wap = "UPDATE cptp_product SET product_wap_img_url='{$img_wap_url}' WHERE product_id='{$product_id}'";
