...
$news_summary = mb_substr(htmlspecialchars($_POST["news_summary"]),0,150,'utf-8');
...
if (isset($file_pc_url)) {
    $sql_img = "UPDATE cptp_news SET news_img_url='{$img_pc_url}' WHERE news_id='{$news_id}'";
    $sql_img_finish = mysqli_query($link, $sql_img);
}

if (isset($file_wap_url)) {
    $sql_img = "UPDATE cptp_news SET news_wap_img_url='{$img_wap_url}' WHERE news_id='{$news_id}'";
    $sql_img_finish = mysqli_query($link, $sql_img);
}

$sql_news = "UPDATE cptp_news SET news_title='{$news_title}', news_click='{$news_click}', news_summary='{$news_summary}', news_all='{$news_all}',news_class='{$news_class}' WHERE news_id='{$news_id}'";
